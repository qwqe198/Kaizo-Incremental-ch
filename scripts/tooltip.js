...
var tooltips, hover_tooltip, tooltip_id = '', tooltip_div;
    ...
    if (elem) {
        if (!el_tooltips.has(elem)) {
            elem.onmouseenter = function() {
                hover_tooltip = elem
                tooltip_div.innerHTML = el_tooltips.get(elem)
...
function updateTooltips() {
    ...
    if (hover_tooltip) {
        let attr_html = el_tooltips.get(hover_tooltip) ?? tooltip_funcs[tooltip_id]?.() ?? ""
        let exist = attr_html !== ''
        ...
        if (!exist) {
            ...
            // console.log(ht_rect, t_rect, attr_pos, attr_dir)
...
function setupTooltips() {
    ...
    setInterval(updateTooltips,1000/FPS)
}